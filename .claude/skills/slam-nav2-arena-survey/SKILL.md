...
```bash
tb3-slam              # cartographer tmux 세션 시작
sleep 8
tb3-ssh
# robot 측에서:
source /opt/ros/jazzy/setup.bash
export ROS_DOMAIN_ID=56 RMW_IMPLEMENTATION=rmw_fastrtps_cpp
ros2 topic hz /map        # 0.5Hz 이상이면 정상
ros2 topic echo /map --once | head -10
```

검증:
- `/map` 토픽 publish rate ≥ 0.5 Hz
- `tb3-rviz` + `tb3-vnc`로 빈 회색 그리드 + 빨간 LiDAR 포인트 보임

문제 발생:
- `/map` 안 나옴 → `tb3-ssh` 후 `tmux a -t slam`으로 cartographer 로그 직접 확인. `Loading parameters from ... cartographer ros... config ... lua` 같은 줄이 나와야 정상.
...
연결 → SLAM → 25분 주행 → 맵 저장 → 평가 → Unity 임포트 → (선택) Nav2 1-waypoint까지 6 Phase. 산출은 `.pgm/.yaml/.png + eval.md` 4종 + Unity 텍스처. 다음 단계는 다중 waypoint 순찰(SCRUM-10) 또는 박물관 액자 보호 트랙(SCRUM-16).
